...
/*
	--------------------------------------------------------------------------------------
	Função para editar um jogo de tabuleiro do servidor via requisição DELETE
	--------------------------------------------------------------------------------------
*/
...
export async function updateGame(id, game) {
	...
	try {
		...
		if (!response.ok) {
			...
			throw new Error(
				JSON.parse(errorMessage).message ||
					'Erro desconhecido ao atualizar o jogo'
			);
		}

		const data = await response.json();
		const editedGame = convertAPIResponseToGameObject(data);

		return editedGame;
	} catch (error) {
		console.error('Erro ao atualizar jogo no backend:', error.message || error);
		throw error;
	}
}
...
/*
	--------------------------------------------------------------------------------------
	Função para pegar sugestão de jogo via IA
	--------------------------------------------------------------------------------------
*/

export async function getAISuggestion(players, user_preferences) {
	const url = `${BASE_URL}/ai/recommendation`;
	const body = {
		players,
		user_preferences,
	};
	...
	try {
		...
		if (!response.ok) {
			...
			throw new Error(
				JSON.parse(errorMessage).message ||
					'Erro desconhecido ao obter sugestão da IA'
			);
		}
		...
		console.error('Erro ao obter sugestão da IA:', error.message || error);
		throw error;
	}
}
...
function convertAPIResponseToGameObject(game) {
	return {
		...
		imageUrl:
			(game?.image_url?.trim() || '').length > 0
				? game.image_url.trim()
				: defaultImage,
		ludopediaUrl: game?.ludopedia_url
			? game.ludopedia_url
			: `${defaultLudopediaUrl}${encodeURIComponent(game.name)}`,
	};
}
